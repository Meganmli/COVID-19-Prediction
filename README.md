...
MLP, Average Ensemble, Product Ensemble, Minimum Ensemble, and Maximum Ensemble each take around 7 minutes to run, and the other classifiers each take less than 5 seconds to run. Note that when running `./run.sh avg`, `./run.sh prod`, `./run.sh min`, or `./run.sh max`, these Ensemble methods include LR, NB, MLP, and RF.
